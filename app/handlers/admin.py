# admin.py
# Admin panel — mahsulotlar, variantlar, buyurtmalar, mijozlar va statistika
...
async def edit_name_received(
    ...
    if not message.text or len(
        ...
        await message.answer(
            "❗ Mahsulot nomini to‘g‘ri kiriting."
        )
        return

    product = await _finish_product_edit(
        message,
        state,
    )
...
async def product_variants(
    ...
    for v in variants:
        builder.button(
            ...
            callback_data=f"admin_variant:{v['id']}",
        )

    builder.button(
        text="➕ Rang qo‘shish",
        callback_data=f"admin_add_variant:{pid}",
    )

    builder.button(
        text="⬅️ Orqaga",
        callback_data=f"admin_product:{pid}",
    )
    ...
    await callback.message.edit_text(
        ...
        + (
            "Rangni tanlang:"
            if variants
            else "Hozircha rang qo‘shilmagan."
        ),
        reply_markup=builder.as_markup(),
    )
...
@admin_router.callback_query(
    F.data.startswith("admin_add_variant:")
)
...
async def variant_name_received(
    message: Message,
    state: FSMContext,
):
...
async def variant_code_received(
    message: Message,
    state: FSMContext,
):
...
@admin_router.callback_query(
    F.data.startswith("admin_variant:")
)
async def variant_detail(
    ...
    builder.button(
        text="✏️ Tahrirlash",
        callback_data=f"edit_variant:{vid}",
    )

    builder.button(
        text="🗑 O‘chirish",
        callback_data=f"delete_variant:{vid}",
    )

    builder.button(
        text="⬅️ Ranglar",
        callback_data=f"product_variants:{v['product_id']}",
    )
...
async def edit_variant_menu(
    ...
    builder.button(
        text="🎨 Rang nomi",
        callback_data=f"edit_v_name:{vid}",
    )

    builder.button(
        text="🔢 Rang kodi",
        callback_data=f"edit_v_code:{vid}",
    )

    builder.button(
        text="🖼 Rasm",
        callback_data=f"edit_v_image:{vid}",
    )

    builder.button(
        text="💰 Narx",
        callback_data=f"edit_v_price:{vid}",
    )

    builder.button(
        text="📦 Qoldiq",
        callback_data=f"edit_v_stock:{vid}",
    )

    builder.button(
        text="⬅️ Orqaga",
        callback_data=f"admin_variant:{vid}",
    )

    builder.adjust(1)

    await callback.message.edit_text(
        f"✏️ <b>{v['color_name']}</b>\n\n"
        "Nimani o‘zgartirmoqchisiz?",
        reply_markup=builder.as_markup(),
    )
